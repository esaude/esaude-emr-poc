...
describe("PatientConfig", function(){
  ...
  xit("should get attributeTypes by UUID", function() {
  ...
  xit("should add autocomplete config for caste", function() {
    var config = new Bahmni.Registration.PatientConfig(samplePersonAttributeTypes());
  ...
  xit("should return heathCentreAttribute", function() {
    var config = new Bahmni.Registration.PatientConfig(samplePersonAttributeTypes());
    expect(config.heathCentreAttribute()).toEqual({uuid: "uuid1", name: "healthCenter"});
  });

  xit("should set showNameField to be false if attribute does not given or middle or family local name", function() {
    var config = new Bahmni.Registration.PatientConfig(samplePersonAttributeTypes());
    expect(config.local().showNameField).toBeFalsy();
  });

  xit("should get the patientAttributeSections through key as section name", function() {
  ...
  xit("should get the ordered patientAttributeSections", function() {
    var config = new Bahmni.Registration.PatientConfig(samplePersonAttributeTypes(),null,patientInfo());
